use crate::vga_buffer::Color::{Black, Red};
use core::fmt;
use core::fmt::Write;
...
const BUFFER_WEIGHT: usize = 80;
#[repr(transparent)]
struct Buffer {
    chars: [[Volatile<ScreenChar>; BUFFER_WEIGHT]; BUFFER_HEIGHT],
}
...
impl Writer {
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            b'\n' => self.new_line(),
            byte => {
                if self.column_position >= BUFFER_WEIGHT {
                    self.new_line()
                }
